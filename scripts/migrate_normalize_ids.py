...
async def migrate_normalize_ids():
    ...
    async with async_engine.begin() as conn:
        ...
        print(f"📊 Found {count_with_dashes} tasks with dashes in notion_page_id")

        if count_with_dashes == 0:
            print("✅ All notion_page_ids already normalized!")
            return

        # Update notion_page_id: remove all dashes
        print("🔄 Normalizing notion_page_id values (removing dashes)...")
        await conn.execute(
            text("UPDATE notion_tasks SET notion_page_id = REPLACE(notion_page_id, '-', '')")
        )

        # Update task id: remove all dashes
        print("🔄 Normalizing task id values (removing dashes)...")
        await conn.execute(
        ...
        if remaining_with_dashes == 0 and ids_with_dashes == 0:
            print(f"✅ Successfully normalized {count_with_dashes} tasks!")
            print("✅ All notion_page_id and id values now have no dashes")
        else:
            print(f"⚠️ Warning: {remaining_with_dashes} notion_page_ids and {ids_with_dashes} ids still have dashes")
...
async def show_sample_tasks():
    ...
    async with async_engine.connect() as conn:
        ...
        print("\n📋 Sample tasks after migration:")
        print("-" * 100)
        for row in result:
            print(f"ID: {row[0][:32]} | notion_page_id: {row[1][:32]} | Title: {row[2]}")

if __name__ == "__main__":
    print("🚀 Starting ID normalization migration...")
    print("=" * 100)
